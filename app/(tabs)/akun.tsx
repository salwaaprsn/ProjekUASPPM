...
export default function AkunScreen() {
    ...
    const handleWhatsApp = () => {
        const url = `https://wa.me/6285759572330?text=Halo%20Salwa,%20mau%20tanya%20dong!`;
    ...
    const handleSimpan = () => {
        try {
            ...
            Alert.alert(t('save'), "Data profil telah diperbarui!");
            setMode('main');
        } catch (error) {
            ...
            <ScrollView style={[styles.bgMain, { backgroundColor: colors.background }]} showsVerticalScrollIndicator={false}>
                <View style={styles.mainWrapper}>
                    ...
                    <View style={[styles.menuSection, { backgroundColor: colors.card }]}>
                        <MenuRow title={t('saved_addresses')} onPress={() => router.push('/alamat' as any)} />
                        <MenuRow title={t('payment_methods')} onPress={() => router.push('/metode_pembayaran' as any)} />
                        <MenuRow title="Pusat Bantuan" onPress={handleWhatsApp} />
                        <MenuRow title={t('settings')} onPress={() => router.push('/pengaturan' as any)} />
                    </View>
                    ...
                    <View style={styles.footer}>
                        ...
                        <TouchableOpacity onPress={handleLogout}><Text style={styles.txtLogout}>{t('logout')}</Text></TouchableOpacity>
                    </View>
                </View>
            </ScrollView>
        );
    ...
    return (
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={[styles.bgWhite, { backgroundColor: colors.background }]}>
            <ScrollView contentContainerStyle={styles.editWrapper}>
                <View style={styles.editHeaderRow}>
                    <TouchableOpacity onPress={() => setMode('main')}><Ionicons name="chevron-back" size={28} color={colors.primary} /></TouchableOpacity>
                    <Text style={[styles.editHeaderTitle, { color: colors.text }]}>Akun Saya</Text>
                    <View style={{ width: 28 }} />
                </View>
                <View style={styles.badgeMemberContainer}>
                    <View style={[styles.badgeMember, { backgroundColor: colors.primary }]}><Text style={styles.badgeText}>Member sejak: {profile.member_sejak}</Text></View>
                </View>
                ...
                <View style={styles.formContainer}>
                    ...
                    <TextInput style={[styles.formInput, { borderBottomColor: colors.border, color: colors.text }]} value={profile.username} onChangeText={(t) => setProfile({ ...profile, username: t })} />
                    ...
                    <TextInput style={[styles.formInput, { borderBottomColor: colors.border, color: colors.text }]} value={profile.no_telepon} onChangeText={(t) => setProfile({ ...profile, no_telepon: t })} keyboardType="phone-pad" />
                </View>
                <TouchableOpacity style={[styles.btnSimpanFull, { backgroundColor: colors.primary }]} onPress={handleSimpan}><Text style={styles.txtBtnWhite}>{t('save')}</Text></TouchableOpacity>
            </ScrollView>
...
const styles = StyleSheet.create({
    ...
    cardWA: { flexDirection: 'row', alignItems: 'center', padding: 15, borderRadius: 15, borderWidth: 1, marginTop: 15 },
    txtWAName: { fontWeight: 'bold' },
    ...
    centerItems: { alignItems: 'center', marginVertical: 10 },
    ...
    formLabel: { fontSize: 12, fontWeight: 'bold', marginTop: 15 },
    formInput: { borderBottomWidth: 1, paddingVertical: 6, fontSize: 14 },
    genderSelector: { flexDirection: 'row', marginTop: 5 },
    ...
    txtBtnWhite: { color: '#FFF', textAlign: 'center', fontWeight: 'bold' }
});